...
const assetsDir =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-gzwhacks/assets';
...
const USER_SCREENSHOTS = [
	...
	},
	{
		...
		file: 'gzw-wallhack-skeleton.webp',
	},
	{
		source:
...
const REMOVE_PREFIXES = [
	'gzw-battle-royale-combat',
	'gzw-extract-fight',
	'gzw-cheats-aimbot',
	'gzw-cheats-cover',
	'gzw-cheats-esp-wallhack',
	'gzw-cheats-hero',
	'gzw-cheats-logo',
	'gzw-cheats-package',
	'gzw-header-art',
...
async function removeOldGZWImages() {
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'gzw-esp-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `gzw-esp-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'gzw-esp-player-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'gzw-cheats-logo.png');
